...
class Load:
    ...
    def readPolyline2(self):
        ...
        for p in range(self.__dataElem.shape[0]):
            pol0 = list(gElem[p].coords.xy)
            for i in range(len(pol0[0]) - 1):
                if pol0[0][i] < min_x:
                    min_x = pol0[0][i]
                if pol0[0][i] > max_x:
        ...
        if abs(max_x - min_x) >= abs(max_y - min_y):
            b = d = 600 / abs(max_x - min_x)
        else:
            b = d = 450 / abs(max_y - min_y)
        for p in range(self.__dataBarr.shape[0]):
            ...
            for i in range(len(pol[0])):
                polyBarr.append(QPointF((pol[0][i] + a) * b + 400, (-pol[1][i] - c) * d + 300))
        ...
        for p in range(self.__dataElem.shape[0]):
            ...
            polyElem = []
            for i in range(len(pol[0])):
                polyElem.append(QPointF((pol[0][i] + a) * b + 400, (-pol[1][i] - c) * d + 300))

            self.polyElem.append(polyElem)

    def readPolyline1(self):
        gBarr = [i for i in self.__dataBarr.geometry]
        max_x = -10000000
        min_x = 10000000
        max_y = -10000000
